...
const log = debug('wp-plugin-index:injector')

const updateIndex = async (scope, compilation, indexPath, injections) => {
	try {
		// load index
		const index = await io.loadSource(indexPath)
		// apply requested injections
		return await injections.reduce(async (acc, injection) => {
			const source = await acc
			const injectorFunc = getInjectorFunc(injection)
			return injectorFunc({
				scope,
				compilation,
				source,
